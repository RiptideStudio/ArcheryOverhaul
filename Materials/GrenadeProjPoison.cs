...
{
	public class GrenadeProjPoison : ModProjectile
	{
		...
		public override void Kill(int timeLeft)
		{
			...
			for (int i = 0; i < 6; i++)
			{
				...
				num = vector.X;
				num2 = vector.Y;
				Projectile.NewProjectile(center.X, center.Y, num, num2, mod.ProjectileType("PoisonProj"), 18, num3, ((Entity)recipe).whoAmI, 0f, 0f);
			}
			for (int j = 0; j < 15; j++)
			{
				Main.rand.Next(-20, 20);
				Dust.NewDust(projectile.position, projectile.width, projectile.height, 74, 0f, 0f, 0, default(Color), 1f);
			}
		}
		...
		public override void AI()
		{
			...
			{
				Dust.NewDust(projectile.position, projectile.width, projectile.height, 75, 0f, 0f, 0, default(Color), 1f);
			}
		}

		public GrenadeProjPoison()
