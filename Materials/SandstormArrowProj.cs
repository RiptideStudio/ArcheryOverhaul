	...
	public class SandstormArrowProj : ModProjectile
	{
		...
		public override void OnHitNPC(NPC target, int damage, float knockback, bool crit)
		{
			...
			for (int j = 0; j < 6; j++)
			{
				...
				x = vector.X;
				y = vector.Y;
				Projectile.NewProjectile(projectile.position.X + (float)num3, ((Entity)recipe).position.Y - 550f - (float)num2, x, y, mod.ProjectileType("SandstormProjLong"), 18, num, projectile.owner, 0f, 0f);
			}
			for (int k = 0; k < 8; k++)
			{
				float x2 = 0f;
				...
				Vector2 vector2 = Utils.RotatedByRandom(new Vector2(x2, y2), (double)MathHelper.ToRadians(60f));
				x2 = vector2.X;
				y2 = vector2.Y;
				Projectile.NewProjectile(projectile.position.X + (float)num6, ((Entity)recipe).position.Y - 550f - (float)num5, x2, y2, mod.ProjectileType("SandstormProjLong"), 18, num4, projectile.owner, 0f, 0f);
			}
		}
